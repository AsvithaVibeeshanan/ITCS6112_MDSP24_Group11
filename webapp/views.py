...
def ixlupload(request):
	if "adminid" in request.session:
		...
		for row in first_sheet.iter_rows(values_only=True):
			...
			d.save()
			# for cell in row:
			# 	print(cell)  # This will print each cell value in the row
...
def psalespredview(request):
	if "adminid" in request.session:
		...
		return render(request, 'psalespred.html', {'yea':years, 'sales': data})
...
def viewsentiment(request):								    
		...
		d=feedback.objects.filter(pid__exact=pid)
		if 'email' in request.session:
			return render(request, 'feedback.html',{'pid':pid,'data':d,'prediction':st})
		else:
			return render(request, 'afeedback.html',{'pid':pid,'data':d})
...
def psales(request):
	...
	# if "adminid" in request.session:
		# data=sales.objects.all()
	return render(request, 'viewproductset.html',{'data':d})
